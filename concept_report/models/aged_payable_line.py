...
class AgedPayableReportLine(models.Model):
    _name = 'aged.payable.report.line'
    _description = 'Aged Payable Report Line'
    _auto = False
    _order = 'date desc'
    ...
    @api.model
    def init(self):
        self.env.cr.execute("""DROP VIEW IF EXISTS aged_payable_report_line CASCADE""")
        self.env.cr.execute("""
            CREATE VIEW aged_payable_report_line AS (
                SELECT
                    aml.id as id,
                    aml.partner_id,
                    aml.move_id,
                ...
                WHERE
                    am.move_type IN ('in_invoice', 'in_refund', 'entry')
